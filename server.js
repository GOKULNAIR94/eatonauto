...
function updateRecord(request, response, result, count, callback){
    ...
    if( count == result.length ){
        ...
        var options = {
          ...
          "port": null,
          "path": "/salesApi/resources/latest/partnerContacts/" + result[count].Id ,
          "headers": {
...
//            console.log("Flag : " + body.PersonDEO_Status_c);
            if( body.PersonDEO_Status_c == "INACTIVE" ){
                result[count].Update = "OK";
                console.log(result[count].Email +","+ result[count].Id +","+ result[count].Update);
                updateRecord( request, response, result, count+1);    
              }
              else{
                  result[count].Update = "Fail";
                console.log(result[count].Email +","+ result[count].Id +","+ result[count].Update);
                updateRecord( request, response, result, count+1);    
              }
...
function deleteRecord(request, response, result, count, callback){
    ...
    if( count == result.length ){
        ...
        var options = {
          ...
          "hostname": "eear-test.crm.us2.oraclecloud.com",
          "port": null,
          "path": "/salesApi/resources/latest/partnerContacts/" + result[count].Id + "/child/userdetails/" + result[count].Email ,
          "headers": {
            "authorization": "Basic RTAwMjgzNjY6UHJvZEAxMjM0",
            "cache-control": "no-cache"
          }
